...
import type { OsirisPaths } from '@richardblaha/osiris-config';
import type { ProviderConfig } from '@richardblaha/osiris-protocol';
import type { Tool } from '@richardblaha/osiris-agent-core';
...
export interface LoadCrewOptions {
  ...
  extraTools?: Tool[];
  /** Expand agent `tools:` selectors like `mcp` / `mcp:<server>` (see `@richardblaha/osiris-mcp`). */
