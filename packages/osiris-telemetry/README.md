...
OTEL_SERVICE_NAME=osiris-web node --import @richardblaha/osiris-telemetry/register ./server/index.mjs
```

`OSIRIS_TELEMETRY=off` forces the no-op path, matching
`@richardblaha/osiris-core`'s telemetry default. Pure ESM, built with `tsc` to `dist/`.
