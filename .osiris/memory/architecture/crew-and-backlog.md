...
- **Crew** (`@richardblaha/osiris-crew`): agents are `.osiris/agents/<name>.md` (YAML
  ...
  fallback for CLI/CI.
- **Memory** (`@richardblaha/osiris-memory`): every `.md` under `.osiris/memory/` is
  ...
  nothing.
- **Backlog** (`@richardblaha/osiris-backlog`): tasks are `[<type>]-<id>-<slug>.md` files;
